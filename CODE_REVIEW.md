...
- **レビュー履歴**:
  ...
  - **v5.6 2026-05-29 — V12/V25 先行実装**（ブランチ `feature/phase2-late-template-dedup`。inline `onclick` を全廃し `templates/_remark_modal.html` を新設して worker.html / index.html の備考モーダル + 送信後 confirm を共通化。worker.html 119→79行、index.html 91→65行に圧縮。pytest 41 件グリーン。詳細は §12 を参照）
  - **v5.7 2026-05-29 — V14 カスタムエラーページ実装**（同ブランチ上で追加。`templates/errors/{403,404,500}.html` と `@app.errorhandler` を追加。スタックトレース露出を防ぎ、職員に「メニューへ戻る」「ログイン画面へ」の導線を出す。pytest 46 件グリーン。詳細は §13 を参照）
- **環境**: Python 3.14.2 / Flask 3.1.0
- **本レビューの範囲**: 報告 → 実装。重大・高リスク項目は実機で再現確認・修正後リグレッション確認済み。

### 現在のステータス（v5.7 時点）

- **フェーズ0/1/2 B案/2.5 完遂、フェーズ2 後半の V12/V25/V14 も先行実装済**。
  フェーズ2.5（v5.5）は main にマージ済（PR #6）。V12/V25/V14 はブランチ
  `feature/phase2-late-template-dedup` 上で pytest 46 件グリーン（main マージ前）。
  詳細は §9・§11・§12・§13 を参照。
- **試用開始の可否**: §7.1 Go/No-Go 判定の **ゲート条件のうちコード側は v5.5 時点で全て満たした**
  状態（v5.6/v5.7 は試用前リスクの追加低減）。残るは現ブランチの PR レビュー・main マージ・
  実機チェック（§8）・運用準備（バックアップ復元演習、パーミッション設定、職員への使い方共有）。
...
| **試用初期** | 日次バックアップ稼働、職員からのフィードバック収集、軽微 UX バグ対応、§10.4 の試用後項目（V13, V15〜V18 ※ V12/V25 は v5.6、V14 は v5.7 で先行完了）を順次対応 | ⏳ |
...
#### 試用開始の Go / No-Go 判定基準（v5.7 時点）

**ゲート条件（すべて満たさないと試用開始しない）**:

コード側（v5.5 で実装完了 + v5.6/v5.7 で前倒し強化）:
...
- ✅ V12 / V25（v5.6）と V14（v5.7）も先行完了 — inline `onclick` 撤廃、テンプレ共通化、カスタムエラーページ
- ✅ `pytest` が全件パス（§8 受け入れテストの自動化版、46 件グリーン）

レビュー・マージ・運用準備（試用前に完了させる）:
- ✅ フェーズ2.5 ブランチを PR 化 → レビュー → main にマージ（PR #6 で完了）
- ⏳ V12/V25/V14 ブランチ（`feature/phase2-late-template-dedup`）を PR 化 → レビュー → main にマージ
- ⏳ 実機（スマホ） + ブラウザで §8 受け入れチェックリストを目視確認
      （pytest がカバーしない JS 動作：V5 confirm 再発火防止、V20 修正フォーム readonly UI、V3 flash 表示、V12/V25 備考モーダル、V14 エラーページの導線）
...
- [ ] 主要操作が監査ログに記録される（§4④）
- ✅ `index.html` と `worker.html` がテンプレ共通化されている（V25）— v5.6 で先行完了、§12 参照
...
| # | 項目 | 状態 |
|---|------|------|
| V12 | inline `onclick` 撤廃（worker.html / index.html 備考モーダル） | ✅ v5.6 で完了（§12） |
| V13 | パスワードルール強化（記号許可、長さ 8+） | 既知。試用後で可 |
| V14 | 500/404/403 のカスタムエラーページ | ✅ v5.7 で完了（§13） |
...
| V25 | `index.html` と `worker.html` のテンプレ重複解消（Codex 第8回） | ✅ v5.6 で完了（§12）。`templates/_remark_modal.html` に備考モーダル + 送信後 confirm + V12 ハンドラを集約 |
...
- ~~V12 — inline `onclick` 撤廃~~ ✅ v5.6 完了
- V13, V15〜V18 — パスワードルール強化、`change_password` UI 補助、`safe_ym` 無音 fallback の警告化、`/login` GET レート制限、等
- ~~V14 — 404/403/500 カスタムエラーページ~~ ✅ v5.7 完了
- ~~V25 — `index.html` と `worker.html` の Jinja `include` 共通化~~ ✅ v5.6 完了
- §4① 希望／確定シフト分離 + `user_id` 化（J の根本対応）
- §4②③④⑤⑥ — 提出状況ダッシュボード、パスワード再発行ワークフロー、監査ログ、開室曜日設定化、CSV/印刷出力

### 11.5 試用開始までの残ステップ

- ✅ フェーズ2.5 ブランチを PR 化 → レビュー → main マージ（PR #6 で完了）
- ⏳ V12/V25/V14 ブランチ（`feature/phase2-late-template-dedup`）を PR 化 → レビュー → main マージ
- [ ] 実機（スマホ + ブラウザ）で §8.1 / §8.2 を目視確認
      （特に v5.6 で追加した V12/V25 関連の JS 動作：備考モーダル開閉、保存後の状態復元、
       送信後 confirm、V5 再発火防止／v5.7 の V14 エラーページ：存在しない URL の 404 表示、
       職員アカウントで `/admin` を叩いた時の 403 表示）
...
## 12. 実装結果（v5.6 — V12 / V25 先行実装）

### 12.1 ブランチ・スコープ

- ブランチ: `feature/phase2-late-template-dedup`
- スコープ: §10.4 試用後でよいに分類されていた V12（inline `onclick` 撤廃）と V25
  （`index.html` / `worker.html` 共通化）を、試用前に前倒し実装。
- 動機: 試用中の軽微な UI 修正コストを下げるため、テンプレ重複と CSP 導入の障壁を
  先に消しておく。試用開始前のリスクは小さい（純粋にリファクタリング、機能変更なし）。

### 12.2 実装内容

| 項目 | 主要変更 |
|------|----------|
| V12 | `worker.html` / `index.html` の inline `onclick="openRemarkModal('{{day}}')"` / `onclick="saveRemark()"` を撤廃。`data-day="{{day}}"` 属性 + `addEventListener` で結線。CSP `script-src 'self'` への移行障壁が解消（CSP 導入は試用後にフェーズ4 で検討）。 |
| V25 | `templates/_remark_modal.html` を新設し、備考モーダル `<div>` + 共通 JS（`openRemark`/`saveRemark` の addEventListener 結線 + 送信後 confirm + V5 `history.replaceState`）を集約。worker.html / index.html はそれぞれ `{% set next_url = url_for(...) %}` + `{% include '_remark_modal.html' %}` で呼び出すだけになり、JS 重複が解消（confirm の遷移先のみテンプレ変数で差し替え）。 |

### 12.3 効果

| ファイル | 行数 | 変化 |
|---------|------|------|
| `templates/worker.html` | 119 → 79 行 | -40 行 |
| `templates/index.html` | 91 → 65 行 | -26 行 |
| `templates/_remark_modal.html` | 新規 64 行 | +64 行 |
| **合計** | **210 → 208 行** | **実質的な dedup は -66 行分の重複コードを 1 箇所に集約** |

- 重複していた JS（`openRemarkModal` / `saveRemark` / submit 後 confirm + `history.replaceState`、計約 40 行）が 1 箇所定義に。
- 今後 modal や confirm の動作を変更する際、両ファイルを同時に書き換える事故が消える。
- inline `onclick` 撤廃により、将来的な `Content-Security-Policy: script-src 'self'` 設定が現実的になった。

### 12.4 検証

`python3 -m pytest` 全 41 件グリーン（v5.5 の 39 件 + V12/V25 用に追加 2 件）：

- `tests/test_shift_input.py::test_index_html_has_no_inline_onclick`
- `tests/test_shift_input.py::test_worker_html_has_no_inline_onclick`

両テストは実 HTML 出力に `onclick=` 文字列が含まれないこと、共通パーシャル経由で
`id="remarkModal"` / `id="modalSaveBtn"` が描画されることを assertion。

JS 動作（モーダル開閉・confirm リダイレクト）は pytest がカバーしないため、
試用前の実機確認項目（§7.1 Go/No-Go）に従ってスマホ・ブラウザで目視確認すること。

### 12.5 試用後の残作業

- V13, V15〜V18（§10.4）— パスワードルール強化、`safe_ym` 無音 fallback の警告化、
  `/login` GET レート制限、`change_password` UI 補助、等。試用 1 か月のフィードバックを
  見てから優先順位付け。V14（404/403/500 カスタムページ）は v5.7 で先行完了済（§13）。
- フェーズ3 — §4① 希望／確定シフト分離 + `user_id` 化（J の根本対応）、§4② 提出状況
  ダッシュボード、§4③ パスワード再発行ワークフロー本格化、§4④ 監査ログ、§4⑤⑥ 開室曜日
  設定化・CSV/印刷出力。

---

## 13. 実装結果（v5.7 — V14 カスタムエラーページ）

### 13.1 ブランチ・スコープ

- ブランチ: `feature/phase2-late-template-dedup`（V12/V25 と同 PR にまとめる方針）
- スコープ: §10.4 試用後でよいに分類されていた V14（403/404/500 のカスタムエラーページ）を
  試用前に前倒し実装。
- 動機: 試用中に職員が「謎の英語ページ」「Werkzeug の Traceback」に遭遇するリスクを消し、
  メニュー / ログインへの導線を必ず出す。
...
| 項目 | 主要変更 |
|------|----------|
| 403 | `templates/errors/403.html` — 「この画面にはアクセスできません」+ `current_user` 有無で「メニューに戻る」or「ログイン画面へ」 |
| 404 | `templates/errors/404.html` — 「ページが見つかりません」+ 同上の導線 |
| 500 | `templates/errors/500.html` — 「サーバー側でエラーが発生しました」「少し時間をおいてもう一度」「繰り返す場合は管理者へ」。`current_user` の参照が失敗するケースに備えて両方の導線を常に表示 |
| app.py | `@app.errorhandler(403/404/500)` を `add_security_headers` 直後に追加（V9 ヘッダはエラーレスポンスにも付く） |

### 13.3 検証

`python3 -m pytest` 全 46 件グリーン（v5.6 の 41 件 + V14 用 5 件）：

- `tests/test_errors.py::test_403_returns_custom_template` — 職員ログインで `/admin` → 403、カスタム HTML
- `tests/test_errors.py::test_404_returns_custom_template_anonymous` — 未ログインで存在しない URL、ログイン導線
- `tests/test_errors.py::test_404_returns_custom_template_logged_in` — ログイン済みで 404、メニュー導線
- `tests/test_errors.py::test_500_returns_custom_template` — 一時 `/__force_500__` ルートで例外発火、Traceback / 例外メッセージが HTML に含まれないこと
- `tests/test_errors.py::test_error_pages_have_security_headers` — エラーレスポンスにも `X-Frame-Options` / `X-Content-Type-Options` が付く

### 13.4 試用後の残作業

§10.4 の試用後項目は V12/V14/V25 完了により以下に縮小:

- V13 — パスワードルール強化（記号許可、長さ 8+）
- V15 — `change_password.html` に flash 表示なし（cosmetic）
- V16 — `safe_ym` の URL 改変時の無音 fallback（警告に変更）
- V17 — `/login` GET のレート制限なし
- V18 — `shifts.name` 連動の根本解消（フェーズ3 §4① で対応予定）
