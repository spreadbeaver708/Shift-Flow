...
app = Flask(__name__)
app.secret_key = "cafe_shift_ultra_final_complete_v10"
...
def change_password():
    if request.method == "POST":
        ...
        if not user or not is_valid_password(p_new):
            conn.close()
            return render_template("change_password.html", error="入力内容が正しくありません")
