...
app = Flask(__name__)
app.secret_key = "cafe_shift_system_2026_final_v11"
        ...
        for week in cal:
            for i, day in enumerate(week):
                if day != 0 and i in [0, 1, 4]:
                    status, remark = request.form.get(f"day_{day}", "×"), request.form.get(f"remark_{day}", "")
                    conn.execute("INSERT INTO shifts (year, month, day, name, status, remarks) VALUES (?,?,?,?,?,?)", (year, month, day, name, status, remark))
        conn.commit()
        conn.close()
        # ★メニューに戻らず、パラメータを付けて自分自身のページにリロード
        base = 'index' if template == "index.html" else 'worker'
        return redirect(url_for(base, name=name, year=year, month=month, submitted="true"))
    conn = get_db()
...
def admin():
    if "username" not in session: return redirect(url_for("login"))
    year, month = request.args.get("year", datetime.now().year, type=int), request.args.get("month", datetime.now().month, type=int)
    conn = get_db()
    # ★INNER JOINでゴースト職員の表示を完全に防止
    rows = conn.execute("SELECT s.day, s.name, s.status, u.color, s.remarks FROM shifts s INNER JOIN users u ON s.name = u.name WHERE s.year=? AND s.month=?", (year, month)).fetchall()
    conn.close()
...
def manage_users():
    ...
    if request.method == "POST":
        ...
        if action == "add":
            ...
            # 名前変更時の連動アップデート
            old = conn.execute("SELECT name FROM users WHERE username=?", (u,)).fetchone()
            if old and old[0] != n:
                conn.execute("UPDATE shifts SET name=? WHERE name=?", (n, old[0]))
            ...
            if u != 'admin':
                user = conn.execute("SELECT name FROM users WHERE username=?", (u,)).fetchone()
                if user: conn.execute("DELETE FROM shifts WHERE name=?", (user[0],)) # ★削除連動
...
def change_password():
    if request.method == "POST":
        u, pc, pn = request.form.get("username"), request.form.get("password_current"), request.form.get("password_new")
        conn = get_db()
        user = conn.execute("SELECT * FROM users WHERE username=? AND password=?", (u, pc)).fetchone()
        if user and len(pn) >= 4 and pn.isalnum():
            conn.execute("UPDATE users SET password=? WHERE username=?", (pn, u))
            conn.commit()
            conn.close()
            flash("パスワードを変更しました")
            return redirect(url_for("login"))
        conn.close()
        return render_template("change_password.html", error="4文字以上の英数文字で入力してください")
