...
2. **バックアップ確認** … Render のディスクは毎日自動でスナップショット（7 日保管）。ただし**DB の復元はスナップショットではなく** Render の「Shell」から取った SQLite バックアップを使う（破損復元を避けるため）:
   ```bash
   sqlite3 /var/data/shift.db ".backup '/tmp/backup.db'"
   sqlite3 /tmp/backup.db "PRAGMA integrity_check;"   # 健全性を確認
   ```
...
| 試用 | 4 名で運用・毎週確認 | ✅ 完了 |
| **次フェーズ（いま）** | **確定シフト・提出状況・操作ログ・セキュリティ強化を追加** | **反映済み** |
| 本運用 | 継続して改善 | 順次 |
